# ini = init  clone but without images rebuild
...
deploy:
	...
	scp -o StrictHostKeyChecking=no -P ${PORT} ${EXCHANGE_RATE_API_KEY_FILE} deploy@${HOST}:site_${BUILD_NUMBER}/secrets/exchange_rate_api_key
	scp -o StrictHostKeyChecking=no -P ${PORT} ${CACHE_PASSWORD_FILE} deploy@${HOST}:site_${BUILD_NUMBER}/secrets/cache_password

	ssh -o StrictHostKeyChecking=no deploy@${HOST} -p ${PORT} 'cd site_${BUILD_NUMBER} && docker login -u ${REGISTRY_USER} -p ${REGISTRY_PASSWORD} ${REGISTRY} && docker stack deploy --compose-file docker-compose.yml finsly --with-registry-auth --prune'
