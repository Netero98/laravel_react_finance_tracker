...
- system_requirements:
  - Кэш по курсам валют один на всех юзеров
  - Тесты нормально выполняются на тестовых временных бд только глобальной командой make test. Если запустить конкретный, то будет использована тестовая бада (в процессе реализации этого). Пришлось делать именно pgsql для автотестов т.к. sql лайт не поддерживает тот же формат миграций. Поэтому нужно сделать in_memory вариант с pgsql для скорости.
  - ПВА работает в соответствии со всеми канонами и хорошими практиками (нужно уточнить детальнее)
